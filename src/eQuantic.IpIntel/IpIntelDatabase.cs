...
/// <summary>
/// An immutable, fully in-memory .eqip dataset. Loading parses the file into
...
/// </summary>
public sealed class IpIntelDatabase
{
    ...
    public static IpIntelDatabase Open(string path)
    {
    ...
    public static IpIntelDatabase Open(Stream stream)
    {
        using var reader = new BinaryReader(stream, Encoding.UTF8, leaveOpen: true);

        if (reader.ReadUInt32() != EqipFormat.Magic)
        {
            throw new InvalidDataException("Not an .eqip dataset.");
        }

        var version = reader.ReadUInt16();
        if (version != EqipFormat.Version)
        {
            throw new InvalidDataException($"Unsupported .eqip version {version}.");
        }
    ...
    private IpInfo LookupV4(uint value)
    {
        ...
        return index >= 0 && value <= _v4Ends[index]
            ? new IpInfo(
                EqipFormat.UnpackCountry(_v4Countries[index]),
    ...
    private IpInfo LookupV6(UInt128 value)
    {
        ...
        return index >= 0 && value <= _v6Ends[index]
            ? new IpInfo(
                EqipFormat.UnpackCountry(_v6Countries[index]),
