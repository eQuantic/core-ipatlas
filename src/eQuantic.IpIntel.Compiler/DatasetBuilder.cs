...
/// <summary>
/// Turns parsed country and ASN ranges into one .eqip file. The two layers
...
public sealed class DatasetBuilder
{
    ...
    /// <summary>Combines both layers and writes the .eqip file.</summary>
    public void Write(Stream output, string source, DateTimeOffset builtAt)
    {
        var v4 = Combine(
            _countries.Where(c => !c.IsV6).Select(c => (c.Start, c.End, Payload: (EqipFormat.PackCountry(c.CountryCode), 0u))),
            _asns.Where(a => !a.IsV6).Select(a => (a.Start, a.End, a.Asn)));
        var v6 = Combine(
            _countries.Where(c => c.IsV6).Select(c => (c.Start, c.End, Payload: (EqipFormat.PackCountry(c.CountryCode), 0u))),
            _asns.Where(a => a.IsV6).Select(a => (a.Start, a.End, a.Asn)));

        EqipFormat.WriteHeader(output, builtAt, source, v4.Count, v6.Count);
        foreach (var segment in v4)
        {
            EqipFormat.WriteV4Record(output, (uint)segment.Start, (uint)segment.End, segment.Country, segment.Asn);
        }

        foreach (var segment in v6)
        {
            EqipFormat.WriteV6Record(output, segment.Start, segment.End, segment.Country, segment.Asn);
        }
    }
